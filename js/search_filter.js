...
function filterProjects() {
    ...
    projects.forEach(project => {
        const title = project.querySelector('.title p').textContent.toLowerCase();
        const projectCategory = project.querySelector('.badges .category').textContent;
       
        if((searchValue === '' || title.includes(searchValue))
            && (filterCategory.includes("All") || projectCategory === filterCategory)) {
...
for (var i = 0; i < filterOptions.length; i++) { // add an event listener to the button
